...
def sin_click():
   global var
   var["result"] = np.sin(np.radians(format_number()))
   update_display(var["result"])
def cos_click():
   global var
   var["result"] = np.cos(np.radians(format_number()))
   update_display(var["result"])
def tan_click():
   global var
   angle = format_number()
   if angle % 180 == 90:  # tan(90°), tan(270°) は無限大
       update_display("Undefined")
   else:
       var["result"] = np.tan(np.radians(angle))
       update_display(var["result"])
